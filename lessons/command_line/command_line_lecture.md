...
> If the command is incomplete in case we forgot to end the command by " in case of echo then nothing will show up.We need to   first complete the command and then enter the ls command to list all the files in that directory
...
> What changed?
> size if files shown in bytes instead of bits
